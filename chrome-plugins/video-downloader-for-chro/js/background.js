...
vd.incrementDownloadCount = function () {
    ...
    if (numberOfDownloads == 3) {
        if (confirm("You have downloaded multiple videos with Video Downloader for Chrome. Please share your experience with others and make a review for us.")) {
            chrome.tabs.create({
                "url": "https://chrome.google.com/webstore/detail/video-downloader-for-chro/dcfofgiombegngbaofkeebiipcdgpnga/reviews",
                "selected": true
            }, function (tab) {
            });
        }
...
vd.showYoutubeWarning = function () {
    ...
    if (confirm("Video Downloader for Chrome is not allowed to download from Youtube.")) {
    }
};
